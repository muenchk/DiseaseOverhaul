	...
	while (iter != itemmap.end() && alchemyEffect != 0) {
		if (Utility::ValidateForm(iter->first) &&
			...
			(acinfo->IsPlayer() == false ||
				acinfo->IsPlayer() &&
					(Settings::NUPSettings::Player::_UseFavoritedItemsOnly == false ||
						std::get<1>(iter->second).get()->IsFavorited()) &&
					(Settings::NUPSettings::Player::_DontUseFavoritedItems == false ||
			...
			if (item && iter->second.first > 0) {
				...
				if (item->IsMedicine() || item->HasKeyword(Settings::GameObj::VendorItemPotion)) {
					...
					if (fortify && acinfo->CanUseFortify(item->GetFormID()) || !fortify && acinfo->CanUsePotion(item->GetFormID())) {
						...
						ret.insert(ret.begin(), { mag, dur, item, AlchemicEffect::kCustom });
							   ...
							   std::get<0>(res) &&
							   (Settings::NUPSettings::Potions::_AllowDetrimentalEffects || std::get<4>(res) == false /*either we allow detrimental effects or there are none*/)) {
...
std::list<RE::AlchemyItem*> ACM::GetAllPotions(std::shared_ptr<ActorInfo> const& acinfo)
{
	...
	while (iter != itemmap.end()) {
		if (iter->first && std::get<1>(iter->second).get() && std::get<1>(iter->second).get()->IsQuestObject() == false) {
			...
			if (item && iter->second.first > 0 && (item->IsMedicine() || item->HasKeyword(Settings::GameObj::VendorItemPotion))) {
	...
	while (iter != itemmap.end() && alchemyEffect != 0) {
		if (Utility::ValidateForm(iter->first) &&
			...
			if (item && iter->second.first > 0) {
				...
				if (item->IsPoison() || item->HasKeyword(Settings::GameObj::VendorItemPoison)) {
					...
					if (acinfo->CanUsePoison(item->GetFormID())) {
						...
						ret.insert(ret.begin(), { mag, dur, item, AlchemicEffect::kCustom });
							   ...
							   std::get<0>(res) &&
							   (Settings::NUPSettings::Poisons::_AllowPositiveEffects || std::get<4>(res) == false /*either we allow poisons with positive effects, or there are no positive effects*/)) {
						ret.insert(ret.begin(), { std::get<1>(res), std::get<2>(res), item, std::get<3>(res) });
					}
				}
			}
...
std::list<RE::AlchemyItem*> ACM::GetAllPoisons(std::shared_ptr<ActorInfo> const& acinfo)
{
	...
	while (iter != itemmap.end()) {
		if (iter->first && iter->second.first > 0 && std::get<1>(iter->second).get() && std::get<1>(iter->second).get()->IsQuestObject() == false) {
			...
			if (item && (item->IsPoison() || item->HasKeyword(Settings::GameObj::VendorItemPoison))) {
...
std::list<std::tuple<float, int, RE::AlchemyItem*, AlchemicEffect>> ACM::GetMatchingFood(std::shared_ptr<ActorInfo> const& acinfo, AlchemicEffect alchemyEffect, bool raw)
{
	...
	while (iter != itemmap.end() && alchemyEffect != 0) {
		if (Utility::ValidateForm(iter->first) &&
			...
			if (item && iter->second.first > 0 &&
				(item->IsFood() ||
					item->HasKeyword(Settings::GameObj::VendorItemFood) ||
					(item->HasKeyword(Settings::GameObj::VendorItemFoodRaw))) &&
				(raw == true || !item->HasKeyword(Settings::GameObj::VendorItemFoodRaw))) {
				...
				if (acinfo->CanUseFood(item->GetFormID())) {
					...
					ret.insert(ret.begin(), { mag, dur, item, AlchemicEffect::kCustom });
						   ...
						   std::get<0>(res) &&
						   (Settings::NUPSettings::Food::_AllowDetrimentalEffects || std::get<4>(res) == false /*either we allow detrimental effects or there are none*/)) {
					ret.insert(ret.begin(), { std::get<1>(res), std::get<2>(res), item, std::get<3>(res) });
				}
			}
		}
...
std::list<RE::AlchemyItem*> ACM::GetAllFood(std::shared_ptr<ActorInfo> const& acinfo)
{
	...
	while (iter != itemmap.end()) {
		if (iter->first && iter->second.first > 0 && std::get<1>(iter->second).get() && std::get<1>(iter->second).get()->IsQuestObject() == false) {
			...
			if (item && (item->IsFood() || item->HasKeyword(Settings::GameObj::VendorItemFoodRaw) || item->HasKeyword(Settings::GameObj::VendorItemFood))) {
			...
			if (poison = std::get<2>(ls.front()); poison) {
				...
				if (ied) {
					...
					acinfo->RemoveItem(poison, 1);
					{
						...
						else if (Settings::GameObj::PoisonUse)
							audiomanager->BuildSoundDataFromDescriptor(handle, Settings::GameObj::PoisonUse->soundDescriptor);
