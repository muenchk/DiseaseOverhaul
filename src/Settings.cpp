...
void Settings::LoadDistrConfigAlchExt()
{
							...
							case 8:  // custom object distribution
								{
										...
										case Settings::Distribution::AssocType::kItem:
											{
												...
												if (tmpb) {
													...
													default:
														{
															...
															if (alch) {
																...
																// item is not an alchemy item so do not add it
																LOGE1_2("[Settings] [LoadDistrRulesAlchExt] custom rule for item {} cannot be applied, due to the item not being an AlchemyItem.", Utility::GetHex(std::get<1>(associtm[i])));
																//citems->items.push_back({ tmpb, std::get<2>(associtm[i]) });
															}
														}
														break;
													...
													LOGE1_2("[Settings] [LoadDistrRulesAlchExt] custom rule for item {} cannot be applied, due to the item not being an TESBoundObject.", Utility::GetHex(std::get<1>(associtm[i])));
												}
											}
											break;
										}
									...
									if (citems->items.size() == 0) {
										logger::warn("[Settings] [LoadDistrRulesAlchExt] rule does not contain any items. file: {}, rule:\"{}\"", file, tmp);
										delete citems;
										continue;
									}
	...
	if (copyrules.size() > 0) {
		for (auto cpy : copyrules) {
			auto splits = std::get<0>(cpy);
			if (splits->size() != 5) {
				logger::warn("[Settings] [LoadDistrRulesAlchExt] rule has wrong number of fields, expected 5. file: {}, rule:\"{}\", fields: {}", std::get<1>(cpy), std::get<2>(cpy), splits->size());
				continue;
			}
			std::string name = (splits)->at(2);
			std::string newname = (splits)->at(3);
			...
			try {
				...
				logger::warn("[Settings] [LoadDistrRulesAlchExt] out-of-range expection in field \"RulePrio\". file: {}, rule:\"{}\"", std::get<1>(cpy), std::get<2>(cpy));
				prio = INT_MIN;
			} catch (std::invalid_argument&) {
				logger::warn("[Settings] [LoadDistrRulesAlchExt] invalid-argument expection in field \"RulePrio\". file: {}, rule:\"{}\"", std::get<1>(cpy), std::get<2>(cpy));
				prio = INT_MIN;
			}
			if (prio != INT_MIN)
				newrule->rulePriority = prio;
			...
			if (newname == DefaultRuleName && (Settings::Distribution::defaultRule == nullptr ||
				...
				Settings::Distribution::defaultRule = newrule;
			delete splits;
			LOGE1_2("[Settings] [LoadDistrRulesAlchExt] rule {} successfully coinialised.", newrule->ruleName);
		}
	}
			...
			if (std::get<0>(a)->size() == 4 || std::get<0>(a)->size() == 5) {
				{
					...
					Settings::Distribution::Rule* rule = Distribution::FindRule(name);
					if (rule == nullptr) {
						logger::warn("[Settings] [LoadDistrRulesAlchExt] rule not found. file: {}, rule:\"{}\"", std::get<1>(a), std::get<2>(a));
						continue;  // rule doesn't exist, evaluate next attachment
					}
					...
					if (std::get<0>(a)->size() == 4)
						...
						try {
							...
							logger::warn("[Settings] [LoadDistrRulesAlchExt] out-of-range expection in field \"RulePrio\". file: {}, rule:\"{}\"", std::get<1>(a), std::get<2>(a));
							continue;
						} catch (std::invalid_argument&) {
							logger::warn("[Settings] [LoadDistrRulesAlchExt] invalid-argument expection in field \"RulePrio\". file: {}, rule:\"{}\"", std::get<1>(a), std::get<2>(a));
							continue;
						}
					}
					...
					for (int i = 0; i < objects.size(); i++) {
						...
						if (EnableLog) {
							...
							case Settings::Distribution::AssocType::kFaction:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached Faction {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated Faction {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							case Settings::Distribution::AssocType::kKeyword:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached Keyword {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated Keyword {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							case Settings::Distribution::AssocType::kRace:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached Race {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated Race {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							case Settings::Distribution::AssocType::kClass:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached Class {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated Class {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							case Settings::Distribution::AssocType::kCombatStyle:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached CombatStyle {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated CombatStyle {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							case Settings::Distribution::AssocType::kNPC:
							case Settings::Distribution::AssocType::kActor:
								if (attach) {
									LOGE3_2("[Settings] [LoadDistrRulesAlchExt] attached Actor {} to rule {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, std::get<1>(a));
								} else
									LOGE5_2("[Settings] [LoadDistrRulesAlchExt] updated Actor {} to rule {} with new Priority {} overruling {}.\t\t\t{}", Utility::GetHex(std::get<1>(objects[i])), rule->ruleName, prio, oldprio, std::get<1>(a));
								break;
							}
						}
					}
				...
				// rule invalid
				logger::warn("[Settings] [LoadDistrRulesAlchExt] rule has wrong number of fields, expected 4 or 5. file: {}, rule:\"{}\"", std::get<1>(a), std::get<2>(a));
				// delet splits since we don't need it anymore
				delete std::get<0>(a);
			}
		}
	...
	if (Settings::EnableLog) {
		...
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of NPCs: {}", Distribution::npcMap()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of NPCs: {}", Distribution::npcMap()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Associations: {}", Distribution::assocMap()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Associations: {}", Distribution::assocMap()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Bosses: {}", Distribution::bosses()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Bosses: {}", Distribution::bosses()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Excluded NPCs: {}", Distribution::excludedNPCs()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Excluded NPCs: {}", Distribution::excludedNPCs()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Excluded Associations: {}", Distribution::excludedAssoc()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Excluded Associations: {}", Distribution::excludedAssoc()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Excluded Items: {}", Distribution::excludedItems()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Excluded Items: {}", Distribution::excludedItems()->bucket_count());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Number of Baseline Exclusions: {}", Distribution::baselineExclusions()->size());
		logger::info("[Settings] [LoadDistrRulesAlchExt] Buckets of Baseline Exclusions: {}", Distribution::baselineExclusions()->bucket_count());
		/*for (int i = 0; i < Settings::Distribution::_rules.size(); i++) {
			logger::info("rule {} pointer {}", i, Utility::GetHex((uintptr_t)Settings::Distribution::_rules[i]));
		}
		auto iter = Settings::Distribution::_assocMap.begin();
